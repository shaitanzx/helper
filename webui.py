            ...
            with gr.Row(elem_classes='extend_row'):
               with gr.Accordion('Extention', open=False):
                  with gr.TabItem(label='OBP') as obp_tab:
                    ...
                    with gr.Row():
                      lock_ar = gr.Dropdown(
                        ["Off", "On", "1:1", "3:2", "4:3", "5:4", "16:9"], 
                        value="Off",
                        label="Lock", show_label=True,interactive=True)
                      lock_ar2 = gr.Dropdown(
                        ["Off", "On", "1:1", "2:3", "3:4", "5:4", "9:16"], 
                        value="Off",
                      ...
                      def locker(lock,width,height):
                        ...
                        if lock != "Off":
                          ...
                          print (height,width,ar_def[0],ar_def[1])
                          return gr.update (interactive=True), gr.update (interactive=False, value=height)
                        return gr.update (interactive=True),gr.update (interactive=True)
                      def swap_ar(lock_ar,lock_ar2,width,height):
                        ...
                        if swap_def:
                          ...
                          return gr.update (visible=False),gr.update (visible=True),gr.update (value=width, interactive=False),gr.update (value=height, interactive=True)
                        height=round (width / ar_def[0] * ar_def[1])
                        return gr.update (visible=True),gr.update (visible=False),gr.update (value=width, interactive=True),gr.update (value=height, interactive=False)
                      ...
                      lock_ar2.change(locker, inputs=[lock_ar,height_ar,width_ar],outputs=[height_ar, width_ar])
