...
def queue_new(*args):
    ...
    clearer()
    return
def args_print(batch_prompt,ctrls_batch):
  asdf=[]
  data = batch_prompt.tolist()
  pandas_df = pandas.DataFrame(data, columns=["prompt", "negative prompt"])
...
def queue_new_prompt(*args):
    ...
    rows=args.pop()
    rows=rows*2
    batch_prompt = args[-rows:]
    args = args[:-rows]
    args = list(args) 
    copy = args[:]
    passed=1
    for index, row in batch_prompt.iterrows():
        prompt = row['prompt']
        negative_prompt = row['negativ prompt']
        args[2]=prompt
...
with shared.gradio_root:
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Row(elem_classes='advanced_check_row'):
                batch_checkbox = gr.Checkbox(label='Batch images', value=False, container=False, elem_classes='min_check')
                prompt_checkbox = gr.Checkbox(label='Batch prompts', value=False, container=False, elem_classes='min_check')
            ...
            with gr.Row(visible=False) as prompt_panel:
                ...
                def args_delete(batch_prompt):
                    if not batch_prompt.empty:
                        batch_prompt = batch_prompt.drop(batch_prompt.index[-1])
                    return batch_prompt
...
#                with gr.Row():
                ...
                prompt_delete.click(args_delete,inputs=batch_prompt,outputs=batch_prompt)
        ...
        ctrls_prompt = tuple(ctrls)
#        for index, row in batch_prompt.iterrows():
#          ctrls_prompt.append([row['prompt'], row['negative prompt']])
#        prompt_start.click(args_print, inputs=[batch_prompt,ctrls_prompt])
...
#                          outputs=[generate_button,prompt_start, prompt_stop]) \
        prompt_start.click(args_print(batch_prompt,ctrls_batch))

              # \
#              .then(prompt_clearer,inputs=batch_prompt,outputs=batch_prompt) \
...
#        (args_print,inputs=batch_prompt)
# .then(lambda: (gr.update(value=f'Add to queue ({len([name for name in os.listdir(batch_path) if os.path.isfile(os.path.join(batch_path, name))])})')), outputs=[add_to_queue])
