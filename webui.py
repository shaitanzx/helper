...
"""
                      ...
                      def locker2(lock,width,height):
                        ...
                        return gr.update (interactive=False, value=height), gr.update (interactive=True)
                      def swap_ar(lock_ar,lock_ar2,width,height):
                        ...
                        if swap_def:                          
                              if lock_ar=="Any":
                                  ...
                                  ratio_x, ratio_y = lock_ar.replace(':', ' ').split(' ')[:2]
                                  lock=str(ratio_y)+":"+str(ratio_x)
                                  print ('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', width,height)
                              return gr.update(visible=False),gr.update (visible=True,value=lock),gr.update(interactive=False,value=width),gr.update (interactive=True,value=height)
                        else:
                          if lock_ar2=="Any":
                            lock="Any"
                          else:
                            ratio_x, ratio_y = lock_ar2.replace(':', ' ').split(' ')[:2]
                            lock=str(ratio_y)+":"+str(ratio_x)
                            print ('bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', width,height)
